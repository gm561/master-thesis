...
\begin{lemma}\textbf{(Security amplification of a dynamic weakly verifiable puzzle with respect to $P_{hash}$.)}
  ...
  If $C$ is such that \\
  \begin{align*}
    \underset{\pi^{(k)}, \rho}{\Pr}[CanonicalSuccess^{P^{(g)}, C, hash}(\pi^{(k)}, \rho)=1] \geq \underset{\mu \leftarrow \mu_\delta^k}{\Pr}[g(\mu) = 1] + \varepsilon,
  \end{align*}
  then $D$ satisfies almost surely
  \begin{align*}
    \underset{\pi, \rho}{\Pr}[\Gamma_V^{(g)}(D^{P^{(1)},\widetilde{C},hash}(\pi, \rho)) = 1] \geq (\delta + \frac{\varepsilon}{6k}).
  \end{align*}
...
%
% \begin{todo}
%   \textbf{TODO:} Write this using expectations to make it shorter
% \end{todo}
%
\begin{proof}
We fix $(\pi_1, \dots, \pi_k)$, $hash$. If $C$ succeeds canonically then
\begin{align*}
...
\begin{align*}
  \underset{\pi^{(k)}, \rho}{\Pr}[CanonicalSuccess^{P^{(g)}, C, hash}(\pi^{(k)}, \rho) = 1] &=
  \sum_{\pi^{(k)} \in \{0,1\}^{kl}}\underset{}{\Pr}[CanonicalSuccess^{P^{(g)}, C, hash}(\widetilde{\pi}^{(k)}) = 1 | \pi^{(k)} = \widetilde{\pi}^{(k)}] \underset{}{\Pr}[\pi^{(k)} = \widetilde{\pi}^{(k)}] \\
  &\leq
  \sum_{\pi^{(k)} \in \{0,1\}^{kl}}\underset{}{\Pr}[\Gamma_V^{(g)} (\widetilde{C}^{\Gamma_V^{(g)}, \Gamma_H^{(g)}, hash}(\widetilde{\pi}^{(k)})) = 1 \mid \pi^{(k)} = \widetilde{\pi}^{(k)}]
  \underset{\pi^{(k)}, \rho}{\Pr}[\pi^{(k)} = \widetilde{\pi}^{(k)}] \\
  &= \underset{\pi^{(k)}, \rho}{\Pr}[CanonicalSuccess^{P^{(g)}, \widetilde{C}, hash}(\pi^{(k)}, \rho) = 1]
\end{align*}
\end{proof}
%
\begin{codeblock}
