...
\end{align}
Let $P^{(g)}, C, (\pi_1, \dots, \pi_k)$ be fixed. We consider the experiment $CanonicalSuccess$ and define a binary random variable $X$ for the event that $hash(q_j) = 0$, and for
...
\end{align*}
Let $G_A$ (correspondingly $G_E$) denote the set of all $(\pi_1, \dots, \pi_k)$ for which $C$ succeeds in the random experiment $Success$ ($CanonicalSuccess$).
For fixed $\pi^{(k)}$, if $C$ succeeds canonically, then it also succeeds in the experiment $Success$.
Hence, $G_E \subseteq G_A$ and we get
\begin{align}
  \label{ineq:hash_high_prob}
\underset{\substack{\textit{hash} \leftarrow \cH \\ \pi^{(k)}}}{\Pr}[CanonicalSuccess^{P^{(g)}, C^{(\cdot, \cdot)}, hash}(\pi^{(k)}) = 1] =
\mathbb{E}_{\pi^{(k)} \in G_A}\left[\underset{\substack{\textit{hash} \leftarrow \cH}}{\Pr}[X = 1]\right]
