...
\end{align}
%
For fixed $hash$ and a problem poser $P$ a canonical success of $C$ for $\pi, \rho$ is a situation when \\ $CanonicalSuccess^{P, C^{(\cdot, \cdot)}, hash}(\pi, \rho) = 1$.
...
such that the success probability of $C$ with respect to $P_{hash}$ in the experiment $CanonicalSuccess$ is at least $\frac{\gamma}{8(h+v)}$.
\end{lemma}
%
\begin{proof}
