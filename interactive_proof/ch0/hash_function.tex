...
\begin{codeblock}
  ...
  \IndII \Return $0$ \\
  \\
  \textbf{If} $(\Gamma_V(q_j, y_j) = 1)$ \And $(\forall i < j :  hash(q_i) \neq 0)$ \And $(hash(q_j) = 0)$ \Then \\
  \IndI \textbf{return} 1\\
  \textbf{else}\\
  \IndI \textbf{return} 0
\end{codeblock}
...
\begin{codeblock}
  ...
  \textbf{Input:} Parameters $\gamma$, $n$. The number of hint $h$ and verification $v$ queries. \\
...
We show that the probability of picking a $hash \in \cH_{Good}$ is at least $\frac{\gamma}{8(h+v)}$.
