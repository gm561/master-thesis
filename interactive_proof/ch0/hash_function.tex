...
$\CanonicalSuccess^{P, C, \hash}(\pi, \rho) = 1$. Clearly, time required to run the experiment $\CanonicalSuccess$ is $O(t)$.
...
Furthermore, FindHash runs in time $\mathit{poly}(h,v,\frac{1}{\gamma},n,t)$,
...
\end{align*}
It is clear that running time of FindHash is $\mathit{poly}(n,h,v,\gamma, t)$ .
\end{proof}
