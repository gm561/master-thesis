...
\begin{definition}[$k$-wise direct-product of DWVPs.]
  ...
  In the first phase, the algorithm $S_1(\rho)$ sequentially interacts in $k$ rounds with $P^{(g)}(\pi^{(k)})$.
...
If $C$ is such that \\
  \begin{align*}
    \underset{\pi^{(k)} \in \{0,1\}^{kn}, \rho \in \{0,1\}^{*}}{\Pr}\left[Success^{P^{(g)}, C}(\pi^{(k)}, \rho) = 1\right]
    \geq 8(h+v)\left(\underset{u \leftarrow \mu_\delta^k}{\Pr}\left[g(u) = 1\right] + \varepsilon\right)
  \end{align*}
then $D$ satisfies almost surely
  \begin{align*}
    \underset{\pi \in \{0,1\}^{n}, \rho \in \{0,1\}^{*}}{\Pr}\left[Success^{P^{(1)},D}(\pi, \rho) = 1\right] \geq (\delta + \frac{\varepsilon}{6k}).
  \end{align*}
