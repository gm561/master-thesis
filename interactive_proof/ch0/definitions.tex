...
%
\begin{codeblock}
  \textbf{Experiment $\mathit{Success}^{P, C}(\pi, \rho) $}
  \medskip
  \hrule
  \medskip
...
\begin{align}
 \underset{\pi, \rho}{\Pr}[\mathit{Success}^{P,C}(\pi, \rho) = 1].
\end{align}
Furthermore, we say that $C$ succeeds for $\pi$, $\rho$ if $\mathit{Success}^{P,C}(\pi, \rho) = 1$.
%
...
and $P_{kn}^{(g)}$ a poser for the $k$-wise direct product of $P_{n}^{(1)}$.
Furthermore, let $C$ be a problem solver for $P_{kn}^{(g)}$ asking at most $h$ hint queries and $v$ verification queries.
There exists a probabilistic algorithm $\mathit{Gen}$ with oracle access to a solver circuit $C$,
