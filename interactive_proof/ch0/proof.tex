...
\begin{lemma}\textbf{(Security amplification for dynamic weakly verifiable puzzles with respect to \boldmath{$\hash$}.)}
  ...
  For fixed $P_n^{(1)}$ let $\widetilde{C} := (C_1, \widetilde{C}_2)$ have oracle access to a function $\hash: Q \rightarrow \{0,1,\dots, 2(h+v-1)\}$
  ...
  has oracle access to $P_n^{(1)}$,  $\widetilde{C}$ for $P_{kn}^{(g)}$, $\hash$, $g: \{0,1\}^{k} \rightarrow \{0,1\}$,
...
\end{align}
Using \eqref{eq:pr_d_succ_0} and inserting \eqref{ineq:first_part} we obtain
\begin{align}
...
\end{align}
then the algorithm Gen recurses almost surely.
