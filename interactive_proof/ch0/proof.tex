...
%
\begin{codeblock}
  $\textbf{Algorithm: EvaluatePuzzles}^{P^{(1)}, P^{(g)}, \widetilde{C}, hash}(\pi^{(k)}, \rho)$
  \medskip \hrule \medskip
...
%
\begin{codeblock}
  $\textbf{Algorithm: EstimateSurplus}^{P^{(1)}, P^{(g)}, \widetilde{C}, g, hash}(\pi^*, b, k, \epsilon, \delta)$
  \medskip
  \hrule
  \medskip
...
Let us define a circuit $\widetilde{C}' = (C_1', \widetilde{C}_2')$ with oracle access $\widetilde{C} = (C_1, \widetilde{C}_2)$.
To make the notation easier we omit the oracle signature, and write $C'$ instead of $C^{C}$.
