...
$(\Gamma_V^{(g)}, \Gamma_H^{(k)}) := \langle P(\pi), C_1(\rho) \rangle_{P}$ we have
\begin{align*}
...
\begin{align*}
  ...
  &\IndII \leq
  \mkern33mu
    \underset{\mathclap{\substack{\pi, \rho \\
        x := \langle P(\pi), C_1(\rho) \rangle_{\text{trans}} \\
        (\Gamma_V^{(g)}, \Gamma_H^{(k)}) := \langle P(\pi), C_1(\rho) \rangle_{P}
      }}}
    {\mathbb{E}}\mkern13mu[\Gamma_V^{(g)}(\widetilde{C}_2^{\Gamma_H^{(k)}, C_2, hash}(x, \rho)) = 1] \\
  &\IndII \leq
  \mkern33mu
    \underset{\mathclap{\substack{\pi, \rho \\
        x := \langle P(\pi), C_1(\rho) \rangle_{\text{trans}} \\
...
\begin{lemma}\textbf{(Security amplification of a dynamic weakly verifiable puzzle with respect to \boldmath{$hash$}.)}
  ...
  \end{align*}
  Additionally, $D$ and $Gen$ require oracle access to $g$, $P^{(1)}$, $\widetilde{C}$.
...
First, we are interested in probability that for $u \leftarrow \mu_{\delta}^k$ and a bit $b \in \{0,1\}$ a function $g$
...
In this algorithm we use circuit $P^{(1)}$ invoking it $k$ times to simulate $k$-rounds of interaction with $C_1(\rho)$.
This requires additional notation. We write $\langle P^{(1)}(\pi_i), C_1(\rho)\rangle^i$ to denote the execution of
the $i$-th round of the simulation, and $\langle P^{(1)}(\pi_i), C_1(\rho)\rangle^i_{P^{(1)}}$
...
We are interested in the success probability of $\widetilde{C}$ when the bitstring $\pi_1$ is fixed to $\pi^*$,
...
If $g$ is the identity function, then the circuit $D^{P^{(1)}, \widetilde{C}}$ returned by \textit{Gen} directly uses $\widetilde{C}$ to find a solution.
...
If $Gen$ manages to find in one of the iterations $\pi^*$ with an estimate that satisfies
$\widetilde{S}_{\pi^*,b} \geq (1-\frac{3}{4k})\epsilon$, we define a monotone function
$g'(b_2, \dots, b_k) := g(b, b_2, \dots, b_k)$, and a circuit $\widetilde{C}' = (C_1', C_2')$, where
...
Let $\cG_{b} := \{ b_1, b_2, \dots, b_k : g(b, b_2, \dots, b_k) = 1 \}$. We observe
\begin{align*}
...
{\Pr}\mkern13mu [\Gamma_V(D^{\widetilde{C}}(x, \rho)) = 1]
...
\underset{
  ...
  \substack{\rho \\ x := \langle P^{(1)}(\pi^*), D^{\widetilde{C}} (\rho) \rangle_{\text{trans}}
    \\ (\Gamma_V, \Gamma_H) := \langle P^{(1)}(\pi^*), D^{\widetilde{C}} (\rho) \rangle_{P^{(1)}} }}}
{\Pr}\mkern13mu[\Gamma_V(D^{\widetilde{C}}(x, \rho)) = 1](S_{\pi^*,1} - S_{\pi^*,0})
\frac{1}{
\underset{u \leftarrow \mu_{\delta}^k}{\Pr}[ u \in \cG_1 \setminus\cG_0]
}
\end{align}
We make use of the fact that the event $c \in \cG_1 \setminus \cG_0$ implies $D_2(x^*, r) \neq \bot$, and write the first summand of (\ref{eq:pr_d_succ_0}) as
\begin{align}
\IndII &\underset{
  \mathclap{
  \substack{\rho \\
