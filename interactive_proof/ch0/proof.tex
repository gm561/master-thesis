...
%
\begin{codeblock}
  \textbf{Circuit $D_2^{P^{(1)}, \widetilde{C}, hash, g, \Gamma_V^*, \Gamma_H^*}(x^*, r)$}
  \medskip \hrule \medskip
...
\begin{codeblock}
  ...
  \medskip \hrule \medskip
  \textbf{Oracle:} A hint oracle $\Gamma_H^{(k-1)}$. \\
  \textbf{Input:}  A tuple $x := (x_1, \dots, x_{k-1}) \in \{0,1\}^{*}$, a bitstring $\rho \in \{0,1\}^{*}$\\
  \textbf{Hard-coded:} A bitstring $\pi^* \in \{0,1\}^{n}$
  \medskip\hrule\medskip
  Simulate $\langle P^{(1)}(\pi^*), C_1(\rho) \rangle^{1}$ \\
  \IndI $(\Gamma_H, \Gamma_V) := \langle P^{(1)}(\pi^*), C_1(\rho) \rangle^{1}_{P^{(1)}}$ \\
  \IndI $x^* := \langle P^{(1)}(\pi^*), C_1(\rho) \rangle^{1}_{\text{trans}}$ \\
  Let $\Gamma_H^{(k)} := (\Gamma_H, \Gamma_H^{1}, \Gamma_H^{2}, \dots, \Gamma_H^{k-1})$ \\
  Let $x^{(k)} := (x^*, x_1, \dots, x_{k-1})$ \\
