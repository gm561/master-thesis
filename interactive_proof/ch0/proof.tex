...
We say that for a fixed $\pi$, $\rho$ the circuit $\widetilde{C}$ \textit{succeeds} if
...
\begin{lemma}\textbf{(Security amplification of a dynamic weakly verifiable puzzle with respect to \boldmath{$hash$}.)}
  ...
  has oracle access to $P^{(1)}$, $P^{(g)}$, $\widetilde{C}$, $hash$, $g$
...
\end{codeblock}
%
All puzzles used by the procedure \textbf{EvalutePuzzles} are generated internally. Thus, the algorithm can answer itself
all queries of $\widetilde{C_2}$ to the hint oracle.
...
We define a circuit $C' = (C_1', \widetilde{C}_2')$ build using the circuit $C$ to which $C'$ has oracle access.
However, to make the notation easier we omit the oracle signature, and write $C'$ instead of $C^{C}$.
...
%
\begin{codeblock}
  \textbf{Circuit $\widetilde{C}_2'(x^{(k-1)}, \rho)$}
  \medskip \hrule \medskip
  ...
  Let $\Gamma_H^{(k)} := (\Gamma_H^*, \Gamma_H^{2}, \dots, \Gamma_H^{k})$ \\
  Let $x^{(k)} := (x^*, x_2, \dots, x_{k})$ \\
  $(q, y_1, \dots, y_k) := \widetilde{C}_2^{\Gamma_H^{(k)}, hash}(x^{(k)}, \rho)$ \\
  \Return $(q, y_2, \dots, y_k)$
\end{codeblock}
%
We are ready to define the circuit $D^{P^{(1)}, P^{(g)}, \widetilde{C}, hash, g} = (D_1^{P^{(1)}, \widetilde{C}}, D_2^{P^{(1)}, P^{(g)}, hash, g})$ and the algorithm $Gen$.
%
\begin{codeblock}
...
%
\begin{codeblock}
  \textbf{Circuit $D_2^{P^{(1)}, P^{(g)}, \widetilde{C}, hash, g, \Gamma_V^*, \Gamma_H^*}(x^*, r)$}
  \medskip \hrule \medskip
  ...
  \IndI $(q, y^*, y_2, \dots, y_k) := \widetilde{C}_2^{\Gamma_H^{(k)}, C, hash}((x^*, x_2, \dots, x_k), \rho)$\\
...
\begin{codeblock}
  ...
  \IndII Let $C_1'$ use $C_1$ and have hard-coded $\pi^*$ \\
  \IndII Let $\widetilde{C}_2'$ use $\widetilde{C}_2$ and have hard-coded $\pi^*$. \\
...
$\delta + \epsilon$. Hence, $D$ trivially satisfies the statement.
...
$C_1'$ first internally simulates the interaction between $C_1$ and $P^{(1)}(\pi^*)$, and use $C_1$ to interact with $P^{(g')}$.
The circuit $\widetilde{C}_2'$ uses $\widetilde{C}_2$, but returns a $(k-1)$-tuple.
