...
\begin{lemma}\textbf{(Security amplification for dynamic weakly verifiable puzzles with respect to \boldmath{$\hash$}.)}
  ...
  Finally, $\mathit{Size}(D) \leq \mathit{Size}(C)\frac{6k}{\varepsilon}$ and $\mathit{Time}(\mathit{Gen}) = \mathit{poly}(k, \frac{1}{\varepsilon}, n)$ with oracle access.
\end{lemma}
%
%
...
%
%
We define now the following circuit $C' = (C_1', C_2')$, which is a solver for the $(k-1)$-wise direct product of $P^{(1)}$.
\begin{codeblock}
...
\end{align}
then Gen recurses almost surely, because probability that
Gen does not find $\widetilde{S}_{\pi, b} \geq (1-\frac{3}{4k})$ in all of the $\frac{6k}{\epsilon}n$ iterations is at most
\begin{align*}
