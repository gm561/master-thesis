...
\begin{lemma}\textbf{(Security amplification of a dynamic weakly verifiable puzzle with respect to \boldmath{$\hash$}.)}
  ...
  has oracle access to $P_n^{(1)}$, $P_{kn}^{(g)}$, $\widetilde{C}$, functions $\hash : Q \rightarrow \{0,1,\dots, 2(h+v-1)\}$, $g: \{0,1\}^{k} \rightarrow \{0,1\}$,
...
\begin{codeblock}
  $\textbf{Algorithm: EvaluatePuzzles}^{P^{(1)}, \widetilde{C}, hash}(\pi^{(k)}, \rho, n, k)$
  \medskip \hrule \medskip
...
%
\begin{codeblock}
  \textbf{Algorithm} $\text{EstimateSurplus}^{P^{(1)}, P^{(g)}, \widetilde{C}, g, hash}(\pi^*, b, k, \epsilon, \delta, n)$
  \medskip
  \hrule
  \medskip
  ...
  \IndI $(c_1, \dots, c_k) := \text{EvalutePuzzles}^{P^{(1)}, P^{(g)}, \widetilde{C}, hash}((\pi^*, \pi_2, \dots, \pi_k), \rho)$\\
...
%
\begin{codeblock}
  \textbf{Circuit $D_2^{P^{(1)}, P^{(g)}, \widetilde{C}, \mathit{hash}, g,  \Gamma_H^*}(x^*, \rho)$}
  \medskip \hrule \medskip
...
%
\begin{codeblock}
  \textbf{Algorithm} $\text{Gen}^{P^{(1)}, P^{(g)}, \widetilde{C}, g, \mathit{hash}}(\epsilon, \delta, n, v, h, k)$
  \medskip \hrule \medskip
  \textbf{Oracle:} Posers $P^{(1)}$, $P^{(g)}$, a solver circuit $\widetilde{C}$ for $P^{(g)}$, functions $g: \{0,1\}^{k} \rightarrow \{0,1\}$, \\
  ...
  \IndI $\widetilde{S}_{\pi^*,0} := \text{EstimateSurplus}^{P^{(1)}, P^{(g)}, \widetilde{C}, g, hash}(\pi^*, 0, k, \epsilon, \delta,n)$\\
  \IndI $\widetilde{S}_{\pi^*,1} := \text{EstimateSurplus}^{P^{(1)}, P^{(g)}, \widetilde{C}, g, hash}(\pi^*, 1, k, \epsilon, \delta,n)$\\
