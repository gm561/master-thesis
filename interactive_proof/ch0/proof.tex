...
\begin{lemma}\textbf{(Security amplification for dynamic weakly verifiable puzzles with respect to \boldmath{$\hash$}.)}
  ...
  For the fixed problem poser $P_n^{(1)}$ let $\widetilde{C} := (C_1, \widetilde{C}_2)$ have oracle access to a function $\hash: Q \rightarrow \{0,1,\dots, 2(h+v-1)\}$
...
The $i$-th round of the interaction between $P^{(1)}$ and $C_1$ is well defined only if all previous rounds have been executed before.
...
is the verification circuit generated by $P^{(1)}$ in the first phase after the interaction with $D_1$.
...
\end{align}
then the algorithm Gen recurses with probability at least $1 - 2^{-n}$, because the probability that
