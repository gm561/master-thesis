If we want to answer hint and verification queries of $C_2$ by ourselves we write $C_2^{(\cdot, \cdot)}$ to stress that $C_2$ does not obtain
...
\begin{lemma}\textbf{(Security amplification of a dynamic weakly verifiable puzzle with respect to \boldmath{$\hash$}.)}
  ...
  For fixed $P_n^{(1)}$ there exists an algorithm $\mathit{Gen}$ that takes as input parameters $\varepsilon$,$\delta$, $n$, $k$,
  has oracle access to $P_n^{(1)}$, $\widetilde{C}$, functions $\hash : Q \rightarrow \{0,1,\dots, 2(h+v-1)\}$, $g: \{0,1\}^{k} \rightarrow \{0,1\}$,
  and outputs a circuit $D := (D_1, D_2)$ such that the following holds: \\
  If $\widetilde{C} := (C_1, \widetilde{C}_2)$ has oracle access to $\hash$ and a solver circuit $C := (C_1, C_2)$ for $P_{kn}^{(g)}$,
  ...
  \end{align*}
  Furthermore, $D$ has oracle access to a hint circuit, $P^{(1)}$, $\widetilde{C}$, $\hash$, $g$, and
...
\begin{codeblock}
  ...
  \textbf{Output:} An estimate $\widetilde{g}$ of $\Pr_{u \leftarrow \mu_{\delta}^{k}}[g(b,u_2, \dotsc, u_k) = 1]$.
  \medskip\hrule\medskip
  \For $i:=1$ \To $\frac{64^2}{\epsilon^2} n$ \Do \\
  \IndI $u \leftarrow \mu_{\delta}^{k}$ \\
  \IndI $g_i := g(b, u_2, \dotsc, u_k)$ \\
  \Return $\frac{\epsilon^2}{64^2n} \sum_{i=1}^{\frac{64^2}{\epsilon^2} n} g_i$
\end{codeblock}
%
\begin{lemma}
  \label{lemma:estimate_of_g}
  The algorithm $\text{EstimateFunctionProbability}^{g}(b, k, \epsilon, \delta)$ outputs an estimate $\widetilde{g}$
...
%
%
The next algorithm $\text{EvalutePuzzles}^{P^{(1)}, P^{(g)}, \widetilde{C}, \hash}(\pi^{(k)}, \rho, n, k)$
...
Therefore, in the algorithm EvalutePuzzles, we use $P^{(1)}$, and invoke it $k$ times to simulate the interaction with $C_1(\rho)$.
...
Furthermore, we write $\langle P^{(1)}(\pi_i), C_1(\rho)\rangle^i_{\mathit{trans}}$ to denote a transcript of communication in the $i$-th round.
%
...
\begin{codeblock}
  ...
  \medskip \hrule \medskip
  \textbf{Oracle:}  Problem posers $P^{(1)}$, a solver circuit $\widetilde{C} = (C_1, \widetilde{C}_2)$ for $P^{(g)}$,\\
  \IndII a function $hash : Q \rightarrow \{0,1,\dots, 2(h+v)-1\}$.\\
...
\end{codeblock}
%
All puzzles used by the algorithm EvalutePuzzles are generated internally.
...
%
\begin{codeblock}
  \textbf{Circuit} $D_1^{\widetilde{C}}(\rho)$
  \medskip \hrule \medskip
  \textbf{Oracle:} A solver circuit $\widetilde{C} = (C_1, \widetilde{C}_2)$ for $P^{(g)}$.\\
  \textbf{Input:} A tuple $\rho := (\tau, \sigma)$ where $ \rho \in \{0,1\}^{*}$ and $\sigma \in \{0,1\}^{*}$.
  \medskip\hrule\medskip
  Run the first round of interation with a problem poser $\langle P^{(1)}, C_1(\tau) \rangle^1$. \\
  Let $x^* := \langle P^{(1)}, C_1(\rho) \rangle^1_{\mathit{trans}}$.
\end{codeblock}
%
\begin{codeblock}
  \textbf{Circuit} $D_2^{P^{(1)}, \widetilde{C}, \mathit{hash}, g,  \Gamma_H^*}(x^*, \rho)$
  \medskip \hrule \medskip
  ...
  %
  \textbf{Input:} A communiation transcript $x^* \in \{0,1\}^{*}$, a bitstring $\rho := (\tau, \sigma)$ \\
  \IndII where $\tau \in \{0,1\}^{*}$ and $\sigma \in \{0,1\}^{*}$\\
  ...
  \IndIII $x_i := \langle P^{(1)}(\pi_i), C_1(\tau) \rangle^i_{\mathit{trans}}$ \\
