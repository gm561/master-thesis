...
\begin{frame}[t]
  \frametitle{Goal}
  \begin{itemize}
    \item Define puzzles that generalize MAC, CAPTCHA, bit commitments.
    \item Hardness amplification result for these puzzles.
  \end{itemize}
  \vspace{40pt}
  \begin{figure}
    \centering
    \begin{tikzpicture}[remember picture,overlay]
      ...
      \node (rect) at (-4,0) [draw,thin, minimum width=3cm,minimum height=2.6cm, text width = 3cm] {
        Weakly Verifiable
      };
      \node (plus) at (-2,0) {+};
      \node (rect) at (0,0) [draw,thin, minimum width=3cm,minimum height=2.6cm, text width = 3cm] {
        Dynamic
      };
      \node (plus) at (2,0) {+};
      \node (rect) at (4,0) [draw,thin, minimum width=3cm,minimum height=2.6cm, text width = 3cm] {
...
\begin{frame}[t]
  \frametitle{Dynamic interactive puzzles}
  \begin{itemize}
    \item Cannot run the solver multiple times.
    \item Hint queries from previous runs can prevent verification queries from succeeding.
