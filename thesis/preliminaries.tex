...
We make sure that it is clear from the context that probabilistic circuits and families of probabilistic circuits with only auxiliary input
are not confused with deterministic Boolean circuits and families of Boolean circuits respectively.
...
as a bitstring provided as an auxiliary input. Exemplary, for a probabilistic algorithm $B$ we write $B(x;r)$
where $x$ is input of $B$ and $r$ denotes a randomness taken as auxiliary input.
...
% \begin{todo}
%   \textbf{TODO:} Time complexity of the interactive protocol. is it needed?
% \end{todo}
...
We underline this explicitly by writing that an algorithm has a certain time complexity \textit{with oracle calls}.
On the other hand, in some situations we are interested in giving a more fine graded bounds on the running time of algorithms
that take into account the time of oracle access. In these situations we write time needed to call an oracle explicitly
and compute running time of an algorithm with regard to time needed for accessing oracle.
