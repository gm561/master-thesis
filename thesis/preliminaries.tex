...
If a probabilistic circuit takes only auxiliary input, we slightly abuse notation and write $C_{n}(r)$.
...
for random bitstrings $\rho_A \in \{0,1\}^{n}$, $\rho_B \in \{0,1\}^{n}$ in the first round $m_0 := A_n(\rho_A)$ and in the second round $m_1 := B_n(\rho_B, m_0)$.
...
On the other hand, in some settings we are interested in giving a more rigorous bounds on the running time of an algorithm.
In these situations we compute the running time explicitly with regard to time needed for accessing oracle.
% \begin{todo}
