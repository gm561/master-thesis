...
and has oracle access to fixed $\widetilde{C}$ and $g$ defined as in $\widetilde{Gen}$.
Additionally, $\widetilde{D}$ has oracle access to $\hash$ and $P^{(1)}$.
...
\begin{codeblock}
  ...
  \Return $\widetilde{D}^{(\cdot), \widetilde{C}, (\cdot), g}$ \textit{//where access to $\widetilde{C}$ and $g$ is fixed}
\end{codeblock}
...
\begin{align*}
  ...
  &\IndII = \Pr \Bigl[\Bigl|\Bigl(\frac{1}{N} \sum_{i=1}^N K_i \Bigr) - \mathbb{E}_{u \leftarrow \mu_{\delta}^k}[g(b,u_2, \dots, u_k)]\Bigr|
    \geq \frac{\epsilon}{8k} \Bigr] \leq 2 \cdot e^{-n/2}.
\end{align*}
\end{proof}
%
...
%In the recursive calls of $\widetilde{\Gen}$ we fix the randomness used to generate the puzzle on the first position.
...
We need to show that the fact that $\widetilde{\Gen}$ fails to find a surplus estimate that is large implies that
that there are only few surpluses greater than $(1-\frac{1}{k})\epsilon$ and their influence
cannot be substantial. Additionally, we have to show that $\widetilde{D}$ outputs an answer often.

The proof requires slightly complex probability manipulations. In Appendix \ref{st:proofSimAssm} we give a shorter proof
under the assumptions that $\widetilde{D}$ always outputs an answer and all surpluses are low.
...
\frac{1}{\underset{u \leftarrow \mu_{\delta}^k}{\Pr}[ u \in \cG_1 \setminus\cG_0]}.
\end{align}
We take a closer look at the first summand of \eqref{eq:pr_d_succ_0}. We know that
\begin{align}
...
$\widetilde{\Gen}$ does not find $\widetilde{S}_{\pi, b} \geq (1-\frac{3}{4k})\epsilon$ in all of the $\frac{6k}{\epsilon}n$ iterations is almost surely at most
\begin{align*}
  \Bigl(1 - \frac{\epsilon}{6k}\Bigr)^{\frac{6k}{\epsilon}n} \leq e^{-n},
\end{align*}
where we used Lemma \ref{lemma:surplus_estimate}.
Therefore, under the assumption that $\widetilde{\Gen}$ does not recurse it almost surely holds
\begin{align}
...
\underset{
  ...
  \substack{r \\ x^* := \langle P^{(1)}(\pi^*), D_1(r) \rangle_{\mathit{trans}}
    \\ (\Gamma_V, \Gamma_H) := \langle P^{(1)}(\pi^*), D_1 (r) \rangle_{P^{(1)}} }}}
{\Pr}\mkern13mu[\Gamma_V(\widetilde{D}_2(x^*, r) = 1]
  (S_{\pi^*,1} - S_{\pi^*,0}) \bigm| \pi^* \notin \cW \Bigr] \Pr_{\pi^*}[\pi^* \notin \cW]\notag\\
