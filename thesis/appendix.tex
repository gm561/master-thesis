...
and in \eqref{pr:low_surpluses} we make an assumption that all surpluses are low.
...
to obtain the desired result. The following calculations are intended to give the Reader more intuition.
We have
\begin{align*}
\underset{
  \mathclap{
...
{\Pr}[\Gamma_V(D_2(x^*,\rho)) = 1 \mid D_2(x,\rho) \neq \bot] \\
  &\overset{\hphantom{a}(*)\hphantom{a}}{=} \underset{\pi^{(k)}}{\Pr}[c_1 \mid c \in \cG_1 \setminus \cG_0] \\
