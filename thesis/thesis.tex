...
In the previous chapter we outlined the former studies of different types of weakly verifiable puzzles and defined
a more general notions of dynamic interactive weakly verifiable puzzles.
The focus of this chapter is on giving a constructive proof of hardness amplification for DIWVP.
First, in Section \ref{st:main_theorem} we formulate the main theorem which is then proved in the succeeding steps.
We start with providing an algorithm that finds an efficiently computable function
that can be used to partition the domain on which hint and verification queries are asked.
Next, we give a proof of hardness amplification under the assumption that the domain is partitioned to advice and attacking queries.
Finally, in Section \ref{st:put_together} we combine these steps which yields the desired result.
%
% \section{Main theorem}
% \label{st:main_theorem}
\subsection{Direct product of interactive weakly verifiable puzzles}
