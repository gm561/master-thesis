...
Different types of weakly verifiable cryptographic primitives have been studied in a series of works \cite{canetti2004hardness},
...
We provide the definition of weakly verifiable puzzles (WVP) that closely follows the one contained in \cite{canetti2004hardness},
and state the theorem of hardness amplification of weakly verifiable puzzles.
...
a puzzle--generation probabilistic algorithm $G$ and a deterministic verification algorithm $V$.
...
The definition of a notion equivalent to the $n$--fold repertion of weakly verifiable puzzles is defined in Section \ref{st:main_theorem}.
...
We note that we take into account the time needed for oracle calls to $S^{(n)}, V, G$ in the analysis for running time of $CHS\text{--}solver$.
%
\begin{codeblock}
