...
Different types of dynamic interactive weakly verifiable puzzles has been analyzed in a series of papers \cite{canetti2004hardness}, \cite{} and \cite{}.
This section give the short overview of the techniqcs used in these works, but also is aimed to give more intuition and insight into
...
%
% note we can not use $k$ as the length of the random bitstring as it is bounded by poly(k)
% (it seems not to be a huge different if I define a polynomial family of circuits)
...
and running time $T$,  then there exists a solver $S$ for $G$ with success probability $\delta(1-\frac{1}{q})$ with running time
$O(\frac{nq^3}{\delta^{2n-1}}(T + nT_G + nT_V))$.
\end{theorem}
\begin{todo}
  \textbf{TODO:} Comparison of our definition and the definition of CHS
  -- mention there is no g
  -- compare circuits with the algorithms
\end{todo}

\begin{todo}
  \textbf{TODO:} Write down the algorithm of CHS.
\end{todo}
