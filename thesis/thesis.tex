...
%
\begin{theorem}[Hardness amplification for Weakly Verifiable Puzzles]
  \label{th:wvp_chs}
...
We give now the intuition why the above algorithm works. The detail proof is contained in \cite{canetti2004hardness}.
First we make the following observation. Let us consider the $n$-fold repetition of $P$ and build a matrix
such that the columns are labeled with all possible puzzles that can occur on the first position. The rows corresponds
to all possible tuple $(p_2, \dotsc, p_n)$.  A cell of the matrix contains a binary $n$-tuple such that
$i$-th bit equals to one if the puzzle on the $i$-th position is solved correctly by the algorithm $A$.
We make the following claim
\begin{observation}
\label{obs:wvp_matrix}
For an algorithm $A$ that solves $n$-fold repetition of $P$ with probability at least $\delta^{n}$.
The matrix $M$ defined as above has either a column with $\delta^{(n-1)}$ fractions of entries that are all one vectors or
a conditional probability that an entry of the form $(1, 1)$ given the entry $(\cdot, 1)$ is at least $\delta$.
\end{observation}
%
% /TODO put proof in appendix ? but the proof is trivial!
%
We see that the algorithm $\mathit{CHS\text{-}solver}$ uses exactly the intuition contained in Observation \ref{obs:wvp_matrix}.
It start with the first position and tries to find fix a puzzle such that success probability of $A$ on the remaining $(n-1)$
position is at least $\delta^{(n-1)}$. If it is possible to find such a puzzle then we fix a puzzle on this position and move to the
next position. If it fails to find such a puzzle, then we assume that there is no column that contains $\delta^{(n-1)}$ fraction
of entries on which $A$ succeeds with the first puzzle fixed. Thus, we following the intuition of Observation \ref{obs:wvp_matrix}
we assume that conditional probability of solving the first puzzle given that all puzzles on all remaining position are solve correctly
is at least $\delta$.

Obviously, the algorithm $\mathit{CHS\text{-}solver}$ may still fail, as it may happen that it does not find a column in
the with high fraction of puzzles solved successfully, although such a column exists.
...
It is not hard to show that all this events happens with low probability. Therefore, with high probability
the algorithm $\mathit{CHS\text{--}solver}$ solves the puzzle correctly with probability at least $\delta$.
%
