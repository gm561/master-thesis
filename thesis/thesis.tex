...
If for a certain DIWVP the number of hint and the number of verification queries is greater than zero, we say that such
...
We say that a DIWVP is \textit{interactive} if in the first phase the number of messages exchanges between a problem poser and a problem
...
Loosely speaking, the message authentication codes may be seen as follows.
...
The sender appends to every message a tag that is computed as a function of the key and the message.
The receiver, using the key, has a way to check whether the appended tag is valid for the received message.
We say that the receiver \textit{accepts} the message if the tag is valid for the received message. Otherwise, the receiver \textit{rejects}.
...
\begin{definition}[Message Authentication Codes]
  ...
  We define the \textnormal{message authentication code (MAC)} as a function efficiently computable function $\times \cK \rightarrow \cT$.
  ...
  a tag for a message $m$ where a key is fixed to $k$. We say that MAC is secure if there is no probabilistic polynomial time algorithm with oracle access to $\Gamma_H$
  ...
  $m'$ and $t'$ such that $\Gamma_V(m',t') = 1$.
\end{definition}
%
We show how a task of breaking the security of MAC is captured by the dynamic interactive weakly verifiable puzzles.
...
We show now that breaking the security of public key signature schemes defined as above can be represented as a dynamic interactive weakly verifiable puzzle.
In the first phase the problem poser uses algorithm $G(1^n)$ to obtain $k_{pub}$ and $k_{priv}$, the public key $k_{pub}$ is sent to the adversary.
...
and is given oracle access to $\Gamma_V$ and $\Gamma_H$.
...
Public key signature schemes are type of puzzles that are dynamic but are not interactive as in the first phase only a single message is sent.
%
...
Additionally, in the \textit{reveal phase} it should be hard for the sender to persuade the receiver that it was committed to the value $\neq b$.
...
For a security parameter \nolinebreak{$n \in \N$} a \textnormal{bit commitment protocol} is defined by a pair $(S_n, R_n)$
