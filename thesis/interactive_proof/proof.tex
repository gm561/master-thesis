...
%
\begin{todo}
  \textbf{TODO:} It is undefined what does it mean to solve a single puzzle of the k--wise direct product succesfully
\end{todo}
...
We write $\pi_1$ to denote randomness used by the problem poser and define $c_1 := \Gamma_V(q,y_1)$ where $\Gamma_V$ is the verification circuit generated
by $P^{(1)}(\pi_1)$ in the first phase when interacting with $D_1(r)$. We note that the problem solver does not know $\pi_1$ and has only oracle access to $\Gamma_V$.

Let us introduce additional notation. We write $c := (c_1, c_2, \dotsc, c_k)$, and for $b \in \{0,1\}$
we define a set $\cG_{b}~:=~\{(b_1, b_2, \dots, b_k) : g(b, b_2, \dots, b_k) = 1 \}$.
