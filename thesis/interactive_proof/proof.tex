...
\begin{lemma}\textbf{(Hardness amplification for a dynamic interactive weakly verifiable puzzle with respect to \boldmath{$\hash$}.)}
  ...
  Finally, $\mathit{Time}(\mathit{Gen}) = \mathit{poly}(k, \frac{1}{\varepsilon}, n)$ with oracle calls.
\end{lemma}
We note that the circuit $D$ from Lemma \ref{lemma:sec_amp_for_p_hash} instead of asking verification queries outputs a pair $(q, y)$
such that $\hash(q) = 0$ or $\bot$.
...
The procedures used by $\Gen$ are presented on the succeeding code listings.
\begin{codeblock}
...
\end{align}
Clearly, the running time of $\Gen$ is $poly(k, \frac{1}{\epsilon}, n)$ with oracle calls.
Furthermore, the algorithm $\Gen$ outputs a circuit $D$ such that it satisfies
with probability at least $1 - \poly(k, \frac{1}{\epsilon}, n) \cdot 2^n$
the statement of Lemma \ref{lemma:sec_amp_for_p_hash}.
This concludes the proof of Lemma~\ref{lemma:sec_amp_for_p_hash}.
\end{proof}
%
%%% Local Variables:
%%% mode: latex
