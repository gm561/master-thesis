...
In inequalities \eqref{ineq:hash_bad_upper} and \eqref{ineq:hash_good_upper} in steps denoted
with $(*)$ we used the trivial facts that $h + v \geq 1$ and $\gamma \leq 1$.
...
Therefore, we conclude that the probability of choosing $\hash \in \cH_{\mathit{Good}}$ amounts at least $\frac{\gamma}{8(h+v)}$.
