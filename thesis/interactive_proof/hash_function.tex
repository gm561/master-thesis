...
For fixed $P_n$ let $C$ be a solver for $P_n$ with the success probability at least $\gamma$, asking at most $h$ hint queries and $v$ verification queries.
Let $\cH$ be a family of pairwise independent efficient hash functions $Q \rightarrow \{0,1, \dots,2(h+v)-1\}$.
...
for every $q_k, q_l \in Q$ such that $q_k \neq q_l$ we have
\begin{align}
 \label{eq:hash_pr}
