...
if it asks a successful verification query $(q_j, y_j)$ such that $\hash(q_j) = 0$, and no hint query $q_i$ is asked before $(q_j, y_j)$ such that $\hash(q_i) = 0$.
%
...
Moreover, let $\cH$ be a family of pairwise independent efficient hash functions $Q \rightarrow \{0,1, \dots,2(h+v)-1\}$.
...
we have that for every $q_k, q_l \in Q$ such that $q_k \neq q_l$ the following holds
\begin{align}
 \label{eq:hash_pr}
