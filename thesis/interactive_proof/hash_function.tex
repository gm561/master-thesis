...
We define the following experiment $\CanonicalSuccess$ in which the set $\cQ$ is partitioned using a function $\hash$.
...
\begin{codeblock}
  ...
  \IndII a function $\hash: \cQ \rightarrow \{0, \dots, 2(h+v) - 1\}$.\\
...
%
\begin{lemma}\textbf{(\boldmath{Success probability in solving dynamic interactive weakly verifiable puzzle with respect to a function $\hash$.)}}
...
Then there exists a probabilistic algorithm FindHash that takes as input parameters $\gamma$, $n$, $h$, $v$, and has oracle access to $C$ and $P_n$.
...
\end{lemma}
%
\begin{proof}[of Lemma \ref{lemma:hash_function_probability}]
...
Let $(q_j, y_j)$ denote the first query such that $\Gamma_V(q_j, y_j) = 1$, then we have
\begin{align}
...
Let us define independent, identically distributed, binary random variables $X_1, \dots, X_{N}$ such that
\begin{align*}
  X_i =
  \begin{cases}
...
\end{align}
Using \eqref{ass:hash_max_prob} we conclude that it is possible to bound the probability of canonical success as follows
\begin{align*}
...
\end{align*}
It is clear that running time of FindHash is $\mathit{poly}(n,h,v,\gamma)$ with oracle calls.
