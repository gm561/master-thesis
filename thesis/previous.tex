...
We will formulate now the definition of $n$-fold repetition of Weakly Verifiable Puzzles along the lines of \cite{canetti2004hardness}.
...
Dynamic weakly verifiable puzzles generalize breaking message authentication codes and public signature schemes.
In case of MAC the adversary takes $x$ which is an empty string.
For the public signature schemes $x$ is the public key.

We write $\cH_{\mathit{hint}}, \cV_{verif} \leftarrow S(x; \delta)$
to denote the execution of the solver $S$ with input $x \in \{0,1\}^{*}$ and using randomness $\delta \in \{0,1\}^{*}$
where $\cH_{hint}$ is the set of all hint queries asked by $S$ and by $\cV_{verif}$ is
...
It is worth seeing why the approach presented in the previous section that works well for $n$-fold repetition of WVP
can not be applied in the setting of the $k$-wise direct product of DWVP (moving aside for a moment the issue of solving only a fraction of puzzle successfully).
...
to find a function $\cQ \rightarrow \{0,1,\dotsc 2(h+v)\}$ such that the success probability of $S$ with respect to
...
that outputs a function $\hash : \cQ \rightarrow \{0,1, \dotsc, 2(h+v)\}$
that partitions $\cQ$ to $\cQ_{attack}$ and $\cQ_{adv}$ such that
with probability at least $\frac{\epsilon}{8(h+v)}$ the first successful verification query $(q',a)$ asked by $S$ is such that $q' \in \cQ_{attack}$
...
in Chapter \ref{ch:main_result}, except the domain partitioning, we use a different prove technique.
...
This is a motivation to study the situations where only a fraction of puzzles is solved successfully.
