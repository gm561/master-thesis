...
that does not take as input $1^k$, but just $\rho$ of length $\mathit{poly}(k)$.
...
Finally, in Definition \ref{def:wvp} a verification algorithm takes as input $p$, $c$, $a$.
...
\begin{definition}[$n$-fold repetition of Weakly Verifiable Puzzles]
  ...
  where for each $1 \leq i \leq n$ pair $(p_i, c_i)$ is an independent instance of a weakly verifiable puzzle defined by $G$ with security parameter $k$ and $V$.
...
%
\begin{todo}
  \textbf{TODO:} define coordinates \\
  \textbf{TODO:} use k-wise direct product instead? \\
\end{todo}
%
...
In Chapter \ref{ch:main_result} we are interested in a more general situation where a monotone function $g: \{0,1\}^{n} \rightarrow \{0,1\}$ is used to decide
...
and running time is $T$, then there exists a solver $S$ for $G$ with oracle access to $S^{(g)}$ that success probability is at least $\delta(1-\frac{1}{q})$ and running time
$O\Big(\frac{nq^3}{\delta^{2n-1}}(T + nT_G + nT_V)\Big)$.
\end{theorem}
%
The following algorithm is used in R.Cannetti, S.Halevi, and M.Steiner in the proof of Theorem \ref{thm:wvp_chs}.
It transforms $S^{(n)}$ for $P^{(n)}$ with success probability at least $\delta^{n}$ to solve a single puzzle $P$ with probability
at least $\delta(1  - \frac{1}{q})$. We use a slackness parameter $q$ as it is not possible to achieve
...
%
To make the notation shorter in the following code excerpts we do not write randomness used by $G$ explicitly.
\begin{codeblock}
...
Let us consider the $n$-fold repetition of $P$, and for simplicity a deterministic solver $S^{(n)}$ for $P^{(n)}$.
...
whereas the rows are labeled with all possible tuples $(p_2, \dotsc, p_n)$ where $G^{(n)}$ is executed with different randomness.
