...
We give a short overview of this work, state the definition of a \textit{dynamic weakly verifiable puzzle} (DWVP) that closely follows
the one included in \cite{Dodis:2009:SAI:1530441.1530450}. Finally, we describe important parts of the proof of hardness amplification of DWVPs
...
\begin{definition}[Dynamic Weakly Verifiable Puzzle.]
  ...
  $\alpha \in \{0,1\}^{*}$ is a secret information and $x \in \{0,1\}^{*}$.
  Furthermore, we consider a set $\cQ$ and a probabilistic polynomial time computable verification relation $R$ such that
  $R(\alpha, q, r) = 1$ if and only if $r \in \{0,1\}^{*}$ is a correct answer to $q \in \cQ$ (for some set of indices $\cQ$)
  on the set determined by $\alpha$. Finally, let $H(\alpha, q)$ be a probabilistic polynomial time computable \textit{hint} relation.
  ...
  We write $P := (\cD, R, H)$ to denote a DWVP with a distribution $\cD$ of pairs $(x, \alpha)$,
  and $R$, $H$ being a verification and hint relations respectively.
\end{definition}
%
...
in Definition \ref{def:dwvp} we use a probabilistic problem poser that outputs circuits $\Gamma_H$ and $\Gamma_V$ that corresponds relations $H$ and $V$, respectively.
Furthermore, in the first phase, which is non-interactive, only a bitstring $x$ is sent by a problem poser.
...
We define the \textit{$n$-wise direct product of DWVPs} in the similar way as the $n$-wise direct product of WVPs.
%
...
the hint relation is defined by $H^{(n)}(q, \alpha_1, \dotsc, \alpha_n) := (H(\alpha_1, q), \dotsc, H(\alpha_n, q))$ and
...
In contrast to, defined in the previous section, the $n$-wise direct product of WVP,
here it is enough if the solver succeeds only on a fraction of puzzles.
...
the set of all pairs $(q,r)$ of verification queries asked in the execution of $S$.
...
Then, there exists a probabilistic algorithm $S$ that solves a puzzle defined by $P$ with probability at least $1-\delta$.
...
It is worth seeing why the approach presented in the previous section that works well for the $k$-wise direct product of WVPs
cannot be applied for the $k$-wise direct product of DWVPs (moving aside for a moment the issue of solving only a fraction of puzzle successfully).
For DWVP the algorithm CHS-solver breaks in the $\mathit{OnlinePhase}$ where the solver $S^{(n)}$ can be called multiple times.
It is possible that in one of these runs $S^{(n)}$ asks a hint query on $q$ for
which in one of the later runs a verification query $(q,r)$ is asked
for which the algorithm would return an answer for the input puzzle
(in other words the condition $\forall_{v+1 \leq i \leq n} V(p_i, c_i, a_i) = 1$ is satisfied).
However, the fact that a hint query on this $q$ has been asked before makes it impossible to ask a successful verification query on this $q$.
...
and a set of \textit{advice queries} $\cQ_{\mathit{adv}}$. The idea is to allow a solver for the $k$-wise direct product to ask hint
...
In  the following lemma is proved.
\begin{lemma}
...
It gains oracle access to $S^{(n)}$, $R_{\alpha}$ and $H_{\alpha}$ as well as $\hash$ from Lemma \ref{lemma:hash_function_previous}.
%
\begin{codeblock}
...
The function $\hash$ is used to partition the query domain.
...
is aborted and we go to the next iteration. In this way we make sure that the DWVP-solver
...
Finally, in case where $S^{(n)}$ asks a verification query on $q$ such that $hash(q) = 0$, then
...
This is a motivation to accept solution where only a fraction of puzzles is solved successful.
...
We use a very similar technique to partition domain $\cQ$ into advice and hint queries as presented in \cite{Dodis:2009:SAI:1530441.1530450}.
...
that determines on which coordinates the solver has to succeed in order to successfully solve the $k$-wise direct product of puzzles.
