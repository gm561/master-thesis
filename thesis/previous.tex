...
Furthermore, in the first phase, which is non-interactive, only a bitstring $x$ is sent by the problem poser to the solver.
...
In contrast to, defined in the previous section the $n$-wise direct product of WVP, here it is enough if the solver succeeds only on a fraction of puzzles.
...
the set of all pairs $(q,r)$ of all verification queries asked in the execution of $S$.

With no loss of generality we make the assumption that the solver does not ask
...
cannot be applied for the direct product of DWVPs (moving aside for a moment the issue of solving only a fraction of puzzle successfully).
...
In each iteration of the DWVP-solver the position for $x \in \{0,1\}^{*}$ is chosen uniformly at random.
...
to partition the query domain into $\cQ_{\mathit{attack}}$ and $\cQ{\mathit{adv}}$.
The DWVP-solver check the hint and verification queries made by $S^{(n)}$.
...
a soft decision system is used to decide whether to ask a verification query.
...
We do not give a more thorough description of the proof of \cite{Dodis:2009:SAI:1530441.1530450} as
