...
Let us argue that the Definition \label{def:wvp} is a special case of Definition \ref{def:dwvp}.
First, we see that in Definition \ref{def:wvp} the algorithm $G$ takes as input $1^k$.
That means that the length of the randomness used by $G$ is bounded by some polynomial $p(k)$.
For a fixed $k$, without loss of generality, we can represent the algorithm $G(1^k; \rho)$ as a probabilistic circuit of polynomial size
that takes as input only the randomness $\rho$ of length bounded by some polynomial $p(k)$.
