...
and provide some intuition and insight into the problem of the hardness amplification for weakly verifiable puzzles.
...
where non-dynamic but interactive weakly verifiable puzzles have been studied.
%
...
We give the definition of WVP that uses the notation and terminology used in this Thesis.

\begin{definition}[Weakly Verifiable Puzzles, \cite{canetti2004hardness}]
  \label{def:wvp}
...
We write $G(1^k; \rho)$ to denote that $G$ takes as input a bitstring $1^k$, where $k$ is a security parameter and the randomness $\rho \in \{0,1\}^{*}$.
...
First, we note that for the efficient algorithm $G$ that takes as input $1^k$ the length of the randomness used by $G$ is bounded by some polynomial $p(k)$.
...
in the circuit $\Gamma_V$ from Definition \ref{def:dwvp}. Hence, for fixed $p$ and $c$ the algorithm $V$
corresponds to $\Gamma_V$. The puzzles considered in Definition \ref{def:wvp} are non--dynamic.
Thus, there is no element corresponding to the hint circuit $\Gamma_H$.
Finally, we note that the puzzles described in Definition~\ref{def:wvp} are non--interactive.
%(the only message send in the first phase is the bitstring $p$).

\subsection{The hardness amplification}
We will now give the definition of the $n$-wise direct product of weakly verifiable puzzles along the lines of \cite{canetti2004hardness}.
...
\begin{definition}[$n$-wise direct product of weakly verifiable puzzles \cite{canetti2004hardness}.]
  ...
  $G^{(n)}$ takes as input $1^{kn}$, as an auxiliary input a bitstring $\rho \in \{0,1\}^{*}$
...
For $P^{(n) } := (G^{(n)},V^{(n)})$ when writing a \textit{puzzle on the $i$-th coordinate} we refer to the $i$-th puzzle of the $n$--fold repetition of $P$
(this puzzle corresponds to the one generated by $G^{(n)}_i$,  $V^{(xn)}_i$).

The $n$-wise direct product of weakly verifiable puzzles is solved successfully if and only if all $n$ puzzles are solved successfully.
...
is used to decide which coordinates of the $n$-wise direct product of puzzles have to be solved correctly.
Clearly, we can assume that $g$ is such that puzzles on all coordinates have to be solved successfully which matches the case considered in Definition \ref{def:n-fold-rep}.
...
\begin{theorem}[Hardness amplification for weakly verifiable puzzles \cite{canetti2004hardness}.]
  \label{thm:wvp_chs}
Let $n,q \in \N$ and $\delta: \N \rightarrow (0,1)$ be efficiently computable functions.
...
It turns $S^{(n)}$ for $P^{(n)}$ with success probability at least $\delta^{n}$ into a solver for a single puzzle $P$ with probability
at least $\delta(1  - \frac{1}{q})$. The parameter $q$ is introduced as it is not possible to achieve
...
In contrast to the $n$-fold repetition of puzzles defined in the previous section, here we require the solver to succeed only on a fraction of puzzles.
