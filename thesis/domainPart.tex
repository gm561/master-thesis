...
The circuit~$C$ is allowed to make hint queries on $q \in \cQ_{adv}$ and successful verification queries on $q \in \cQ_{attack}$.
...
Let us now prove Lemma~\ref{lemma:hash_function_probability}.
%
%
...
We~are interested in probability over the choice of $\hash$ of the event $\CanonicalSuccess^{P,C,\hash}(\pi^*, \rho^*) = 1$
where $(\pi^*, \rho^*) \in \cP_{\mathit{Success}}$ is fixed. Let $(q_j, y_j)$ denote the first verification query for which $C$ succeeds.
...
\end{align}
The following algorithm FindHash uses a natural sampling approach to pick a function $\hash$ that satisfies Lemma \ref{lemma:hash_function_probability}.
\begin{codeblock}
...
In the following inequalities \eqref{ineq:hash_bad_upper} and \eqref{ineq:hash_good_upper} in the steps denoted
