%
In the previous chapter we defined dynamic interactive weakly verifiable puzzles(DIWVPs), the $k$-wise direct product of DIWVP,
and stated the hardness amplification theorem for DIWVP.
In this chapter we give an overview of well known cryptographic primitives that can be seen as different types of weakly verifiable puzzles.
First, in Section \ref{section:mac} we describe message authentication codes which game based security definition can be seen being a dynamic weakly verifiable puzzle.
In Section \ref{section:pks} we describe public key signature schemes and give
the chosen-message attack security definition of public key signature schemes which is an example of dynamic weakly verifiable puzzle.
Then, in Section \ref{section:bc} we focus on bit commitment protocols that binding property can be seen as interactive weakly verifiable puzzle.
Finally, in Section \ref{section:att} we describe a problem of solving CAPTCHAs that can be considered to be a weakly verifiable puzzle.
...
\label{section:pks}
In this section we describe \textit{public key signature schemes} which the game base security definition
for the chosen message attack can be seen as a dynamic, non-interactive weakly verifiable puzzle.
...
$n~\in~\N$ is a security parameter. We call $S_n$ the \textit{sender} and $R_n$ the \textit{receiver}.
...
The circuit $\Gamma_H$ takes as input $y_0 \in \{0,1\}^{*}$, $y_1 \in \{0,1\}^{*}$ and outputs $1$
...
and can ask at most a single verification query. In general, for CAPTCHAs checking the correct of a solution is
