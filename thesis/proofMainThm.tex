...
\begin{codeblock}
  ...
  $hash := \FindHash^{P^{(g)}, C}(\gamma, n, h, v)$ \textit{// simulate calls to $P^{(g)}$ using $P^{(1)}$,$g$}\\
  ...
  \Return $D := (D_1, D_2)$ \textit{//with fixed oracle access to $\widetilde{D}_2$}
\end{codeblock}
%
\begin{codeblock}
  \textbf{Circuit} $D_2^{\widetilde{D}_2, \Gamma_V, \Gamma_H}(x, \rho)$
  \medskip \hrule
  ...
  \textbf{Input:}  Bitstrings $x \in \{0,1\}^{*}$, $\rho \in \{0,1\}^{*}$.
  \medskip\hrule
  $(q, y) := \widetilde{D}_2^{\Gamma_H}(x, \rho)$ \textit{//where $D_2$ has oracle access to $\widetilde{C}, \hash, g$ from $\widetilde{\Gen}$} \\
  Ask the verification query $(q,y)$ to $\Gamma_V$.
\end{codeblock}
%
